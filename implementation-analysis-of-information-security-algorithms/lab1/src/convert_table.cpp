...
int main() try
{
    static const table_t ENCRYPTION_TABLE =
    {
        ...
        mask_t{ 0x5F, 0x41, 0x5A, 0x0B, 0x00, 0x5D, 0x45, 0x04, 0x58, 0x44, 0x58, 0x44, 0x56, 0x07, 0x46, 0x00, },
        mask_t{ 0x58, 0x46, 0x5D, 0x0C, 0x07, 0x5A, 0x42, 0x03, 0x5F, 0x43, 0x5F, 0x43, 0x51, 0x00, 0x41, 0x07, },
        ...
        mask_t{ 0x5B, 0x45, 0x5E, 0x0F, 0x04, 0x59, 0x41, 0x00, 0x5C, 0x40, 0x5C, 0x40, 0x52, 0x03, 0x42, 0x04, },
        mask_t{ 0x54, 0x4A, 0x51, 0x00, 0x0B, 0x56, 0x4E, 0x0F, 0x53, 0x4F, 0x53, 0x4F, 0x5D, 0x0C, 0x4D, 0x0B, },
        ...
        mask_t{ 0x09, 0x17, 0x0C, 0x5D, 0x56, 0x0B, 0x13, 0x52, 0x0E, 0x12, 0x0E, 0x12, 0x00, 0x51, 0x10, 0x56, },
        ...
        mask_t{ 0x05, 0x1B, 0x00, 0x51, 0x5A, 0x07, 0x1F, 0x5E, 0x02, 0x1E, 0x02, 0x1E, 0x0C, 0x5D, 0x1C, 0x5A, },
        mask_t{ 0x06, 0x18, 0x03, 0x52, 0x59, 0x04, 0x1C, 0x5D, 0x01, 0x1D, 0x01, 0x1D, 0x0F, 0x5E, 0x1F, 0x59, },
        mask_t{ 0x07, 0x19, 0x02, 0x53, 0x58, 0x05, 0x1D, 0x5C, 0x00, 0x1C, 0x00, 0x1C, 0x0E, 0x5F, 0x1E, 0x58, },
        mask_t{ 0x00, 0x1E, 0x05, 0x54, 0x5F, 0x02, 0x1A, 0x5B, 0x07, 0x1B, 0x07, 0x1B, 0x09, 0x58, 0x19, 0x5F, },
        mask_t{ 0x01, 0x1F, 0x04, 0x55, 0x5E, 0x03, 0x1B, 0x5A, 0x06, 0x1A, 0x06, 0x1A, 0x08, 0x59, 0x18, 0x5E, },
        mask_t{ 0x02, 0x1C, 0x07, 0x56, 0x5D, 0x00, 0x18, 0x59, 0x05, 0x19, 0x05, 0x19, 0x0B, 0x5A, 0x1B, 0x5D, },
        ...
        mask_t{ 0x1E, 0x00, 0x1B, 0x4A, 0x41, 0x1C, 0x04, 0x45, 0x19, 0x05, 0x19, 0x05, 0x17, 0x46, 0x07, 0x41, },
        ...
        mask_t{ 0x19, 0x07, 0x1C, 0x4D, 0x46, 0x1B, 0x03, 0x42, 0x1E, 0x02, 0x1E, 0x02, 0x10, 0x41, 0x00, 0x46, },
        mask_t{ 0x1A, 0x04, 0x1F, 0x4E, 0x45, 0x18, 0x00, 0x41, 0x1D, 0x01, 0x1D, 0x01, 0x13, 0x42, 0x03, 0x45, },
        mask_t{ 0x1B, 0x05, 0x1E, 0x4F, 0x44, 0x19, 0x01, 0x40, 0x1C, 0x00, 0x1C, 0x00, 0x12, 0x43, 0x02, 0x44, },
    ...
    std::array<mask_t, 256> decryption_table;
    for (std::size_t i = 0; i < 256; ++i)
    {
        for (std::size_t j = 0; j < PERIOD_SIZE; ++j)
        {
            decryption_table[ENCRYPTION_TABLE[i][j]][j] = i;
        }
    }

    std::cout << "static const table_t DECRYPTION_TABLE =\n{\n";
    for (std::size_t i = 0; i < 256; ++i)
    {
        std::cout << "    mask_t{ ";
        for (std::size_t j = 0; j < PERIOD_SIZE; ++j)
        {
            std::cout << std::hex << std::internal << std::setfill('0') << std::showbase << std::setw(4) << static_cast<int>(decryption_table[i][j]) << ", ";
        }
        std::cout << "},\n";
    }
    std::cout << "}\n";
